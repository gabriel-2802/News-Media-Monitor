...
def embed_batch(articles: List[ArticleRow]) -> Tuple[List[int], np.ndarray]:
    ...
    return a list of pairs (id, embedding).
    """
    texts = [
...
def cluster_embeddings(ids: List[int], embeddings: np.ndarray, threshold : float = 0.8, k_neighbours : int = 10) -> Dict[int, List[int]]:
    """
    clusters article embeddings using FAISS and cosine similarity.
    returns a dict: cluster_id -> list of article_ids
    """
    embeddings = embeddings.astype(np.float32)

    # initialize a FAISS index using inner product
    ...
    for i in range(len(embeddings)):
        ...
        # search k neighbors for embedding i
...
def save_clusters(clusters: Dict[int, List[int]]):
    """
    creates new clusters, adds them to the DB, and updates associated articles.
    skips clusters with only one article.
    """
        ...
        with conn:
            with conn.cursor() as cursor:
                for cluster_articles in clusters.values():
                    ...
                    # Update article references to cluster (pass as array)
                    cursor.execute(UPDATE_ARTICLE_QUERY, (cluster_id, cluster_articles))  # now safe
